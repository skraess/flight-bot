...
function getFlightDates(req, res) {
    ...
    const searchid = (req.body.conversation.memory['searchid'].scalar != undefined) ? req.body.conversation.memory['searchid'].scalar - 1 : req.body.conversation.memory['searchid'].rank - 1;
    if (searchid + 1 > req.body.conversation.memory.flights.length) {
        res.json({
            replies: [
                {
                    type: 'text', content: `I'm sorry but there is no search result with number ${searchid + 1}.`
                },
            ],
        });
    } else {
